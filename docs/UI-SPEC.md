...
### 1.1.2 Sidepanel-Only Tokens (Chat Bubbles & Layout)

These tokens exist only in `sidepanel.css` and control chat bubble appearance:

| Token | Light Value | Dark Value |
|-------|------------|------------|
| `--model-max` | `160px` | `160px` |
| `--model-min` | `160px` | `160px` |
| `--prompt-max` | `420px` | `420px` |
| `--prompt-min` | `140px` | `140px` |
| `--user-bubble-padding-x` | `12px` | `12px` |
| `--user-bubble-padding-y-multi` | `7px` | `7px` |
| `--user-bubble-padding-y-single` | `4px` | `4px` |
| `--user-bubble-line-height-multi` | `1.36` | `1.36` |
| `--user-bubble-line-height-single` | `1.32` | `1.32` |
| `--user-bubble-radius` | `16px` | `16px` |
| `--assistant-bubble-padding-x` | `18px` | `18px` |
| `--assistant-bubble-padding-y` | `14px` | `14px` |
| `--assistant-bubble-line-height` | `1.50` | `1.50` |
| `--assistant-bubble-radius` | `22px` | `22px` |
| `--message-size` | `14px` | `14px` |
| `--message-weight` | `400` | `400` |
| `--messages-gap` | `12px` | `12px` |
| `--bubble-user` | `rgba(0,0,0,0.05)` | `#3a3c3e` |
| `--bubble-assistant` | `#ffffff` | `#2a2b2d` |
| `--bubble-user-border` | `transparent` | `transparent` |
| `--bubble-assistant-border` | `transparent` | `rgba(255,255,255,0.06)` |
...
| Nav item | `9px 14px` |
| Model row | `10px 14px` desktop, `12px` narrow |
...
- Desktop layout: one horizontal row with `display:flex`, `align-items:center`, `gap:14px`, `padding:10px 14px`, and `border-radius:12px`.
- Model name input uses flexible width (`flex:1`) and should be the primary readable field.
- Custom JSON and Prefix inputs are compact secondary controls (`width:150px`, `font-size:11px`).
- Enable switch and delete icon keep fixed intrinsic size and must not shrink.
- Narrow layout (`<= 880px`): hide `.model-list-header`; allow `.model-row` to wrap; model name uses `flex:1 1 calc(100% - 96px)` with `min-width:180px`; Custom JSON and Prefix use `flex:1 1 140px`; switch and delete button stay `flex:0 0 auto`.

#### Options Toggle Sections

- Toggle sections such as Float Ball, Sidebar Behavior, Auto-generate Chat Titles, and TTS Auto-read use a `.section-header` row with the title on the left and switch on the right.
- Toggle section subtitles use `.section-subtitle` at `15px / 24px`, visually subordinate to the main `.section-title` at `18px / 24px`.
- At `<= 880px`, these sections must override the generic stacked `.section-header` rule and remain `flex-direction:row`, `align-items:center`, `justify-content:space-between`.
- Each toggle section can use a scoped row class, e.g. `.tts-auto-read-row`, and must be included in the `<= 880px` row override when it appears inside a section affected by the generic `.section-header` mobile rule.
- Section title and hint stay `text-align:left`; switches stay `flex:0 0 auto`.
...
| `<= 880px` | Options: card padding reduced, footer stacked, generic section headers stack |
| `<= 880px` scoped overrides | Options toggle sections stay row-aligned with title/hint left and switch right; Enabled Models hides column header and wraps row controls inside the card |
