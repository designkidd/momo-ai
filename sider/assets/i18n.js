...
(function(){
  const translations = {
    'hant': {
      ...
      'pageCaptureHint': '選擇引用頁面的擷取方式。「智能擷取」會自動選擇較省 token 的策略：文章／文件頁使用精簡 Markdown，產品頁、定價頁、dashboard 等應用型頁面使用可見結構文字。「整個頁面」建議只在精簡模式漏內容時使用。',
      'captureMode': '擷取模式',
      ...
      'pageContextLimitHint': '設定每次引用頁面時，最多交給 AI 讀取多少文字（建議 20000-100000）。一般保持預設即可；如果頁面很長或內容常被截斷，再把數值調高。數值越大，AI 看到的內容越多，也會用更多 token。',
    ...
    'hans': {
      ...
      'pageCaptureHint': '选择引用页面的抓取方式。“智能抓取”会自动选择较省 token 的策略：文章/文档页使用精简 Markdown，产品页、定价页、dashboard 等应用型页面使用可见结构文字。“整个页面”建议只在精简模式漏内容时使用。',
      'captureMode': '抓取模式',
      ...
      'pageContextLimitHint': '设置每次引用页面时，最多交给 AI 读取多少文字（建议 20000-100000）。一般保持默认即可；如果页面很长或内容经常被截断，再把数值调高。数值越大，AI 看到的内容越多，也会用更多 token。',
    ...
    'en': {
      ...
      'pageCaptureHint': 'Choose how page references are captured. "Smart Capture" automatically selects a token-saving strategy: reader-style Markdown for articles and visible structured text for product pages, pricing pages, dashboards, and other app-like pages. Use "Full Page" only when the cleaner modes miss important content.',
      ...
      'pageContextLimitHint': 'Set how much page text the AI can read each time you reference a page (recommended: 20000-100000). The default is fine for most pages; increase it only when long pages are being cut off. Higher values show the AI more content and use more tokens.',
